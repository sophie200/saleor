...
def trigger_webhook_for_event_sync(event_type, data):
    ...
    for webhook in webhooks:
        ...
        responses.append({"webhook": webhook.pk, "response": response})
    return responses
...
def send_webhook_request_sync(webhook_id, target_url, secret, event_type, data):
    parts = urlparse(target_url)
    domain = Site.objects.get_current().domain
    message = data.encode("utf-8")
    signature = signature_for_payload(message, secret)
